...
float SlowValueToBoundedFloat(const uint16 value, const uint16 unknown_value,
  ...
  //check if value Less Than kValueCount
...
std::unique_ptr<std::vector<float>> PrecomputeValueToBoundedFloat(
  ...
  //reserve capacity while elements are not added yet
  //resize augement the capacity and add new elements
...
std::vector<uint16> ComputeLookupTableToApplyOdds(const float odds) {
  ...
  for (int cell = 1; cell != kValueCount; ++cell) {
    result.push_back(ProbabilityToValue(ProbabilityFromOdds(
                     ...
                     kUpdateMarker);
  //add UpdateMarker to update each hit
  }
  return result;
}
