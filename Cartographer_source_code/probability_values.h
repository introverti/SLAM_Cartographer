...
inline uint16 BoundedFloatToValue(const float float_value,
  ...
  //            [P(float)-P(float_MIN)]            (2^15-1)-1
  //P(int)= ------------------------------- * -----------------------
  //	      [P(float_MAX)-P(float_MIN)]    [P(int_MAX)-p(int_MIN)]
  const int value =
      common::RoundToInt(
  ...
  DCHECK_GE(value, 1);
  //check if less than 32767
  DCHECK_LE(value, 32767);
  return value;
}

}  // namespace
//oods=p(s=1)/p(s=0)=P/(1-P)
...
//p(s=1) as Probability [0.1,0.9]
constexpr float kMinProbability = 0.1f;
constexpr float kMaxProbability = 1.f - kMinProbability;
//p(s=0) as CorrespondenceCost
...
//if input doesn't belong to the intervelle, then output lateral values
...
//P and C values are precalculated and storen in vector
...
inline float ValueToProbability(const uint16 value) {
  //search in the vector to find correspond value
...
inline uint16 ProbabilityValueToCorrespondenceCostValue(
  ...
  if (probability_value > kUpdateMarker) {
    ...
    update_carry = true;//if oversize
  }
  //P(int)->[search in vector]P(float)->C(float)->C(int)
